...
var enableProfileCommandDefinition = agentstructs.Command{
	Name:        "enable-profile",
	Description: "Enables a profile in the agent.",
	HelpString:  "enable-profile [profile id]",
	Version:     1,
	...
	CommandParameters: []agentstructs.CommandParameter{
		{
			Name:             "id",
			Description:      "ID of the profile to enable.",
			ModalDisplayName: "ID of the profile to enable.",
	...
	TaskCompletionFunctions: map[string]agentstructs.PTTaskCompletionFunction{
		"enableProfilePostRunActions": enableProfilePostRunActions,
	},
...
func enableProfilePostRunActions(
	...
	output := fmt.Sprintf("Enabled profile %d.", int(profileID))
	...
	extraInfo.C2Profiles[profileIndex].Enabled = true
...
func enableProfileParseArgsString(args *agentstructs.PTTaskMessageArgsData, input string) error {
...
func enableProfileCreateTasking(task *agentstructs.PTTaskMessageAllData) agentstructs.PTTaskCreateTaskingMessageResponse {
	...
	if !extraInfo.C2Profiles[profileIndex].Enabled {
		taskResponse.Error = "Profile already enabled."
	...
	completionFunction := "enableProfilePostRunActions"
...
func init() {
	agentstructs.AllPayloadData.Get("thanatos").AddCommand(enableProfileCommandDefinition)
}
