    ...
    def __init__(self, command_line, **kwargs):
        ...
        self.args = [
            ...
            CommandParameter(
                name="exec",
                type=ParameterType.String,
                description="Command used to run the payload ({path} is the path to the payload on the remote system)",
                display_name="Command used to run the payload ({path} is the path to the payload on the remote system)",
        ...
        if uuid := task.args.get_arg("payload"):
            ...
            try:
                ...
                file_resp = await SendMythicRPCFileCreate(
                    MythicRPCFileCreateMessage(
                        task.id,
                        FileContents=base64.b64encode(file_upload.encode()).decode(),
