        ...
        for (var i = 0; i < responses.length; i++) {
            ...
            if (platform == "Linux") {
                headers = [
                    ...
                    { "plaintext": "actions", "type": "button", "cellStyle": {}, "width": 150, "disableSort": true },
                ];
            }
            else {
                headers = [
            ...
            for (var j = 0; j < processes.length; j++) {
                ...
                var start_time = "";
                if (pinfo["start_time"] != null) {
                    var date = new Date(parseInt(pinfo["start_time"]) * 1000);
                    var dow = DAYS_OF_WEEK[date.getDay()];
                    var month = MONTHS[date.getMonth()];
                    start_time = dow + ', ';
                    start_time += String(date.getDate() + ' ');
                    start_time += month + ' ';
                    start_time += String(date.getFullYear() + ' ');
                    start_time += String(date.getHours() + ':');
                    start_time += String(date.getMinutes()).padStart(2, '0') + ':';
                    start_time += String(date.getSeconds()).padStart(2, '0');
                }
                var row = {};
                if (platform == "Linux") {
                    row = {
                                ...
                                "value": [
                                    {
                                        ...
                                        "type": "dictionary",
                                        "value": {
                                            "Process Path": (pinfo["bin_path"] != null ? pinfo["bin_path"] : ""),
                                            "Command Line": (pinfo["command_line"] != null ? pinfo["command_line"] : ""),
                                ...
                                "value": [
                                    {
                                        ...
                                        "value": {
                                            "Process Path": (pinfo["bin_path"] != null ? pinfo["bin_path"] : ""),
                                            "Description": (pinfo["description"] != null ? pinfo["description"] : ""),
                                            "Command Line": (pinfo["command_line"] != null ? pinfo["command_line"] : ""),
                                            "Start Date": start_time,
                                        },
                                        "leftColumnTitle": "Attribute",
